//base javascript for sphinx, this handles most functions

var baseURL             = "https://chatroom-ebf81-default-rtdb.firebaseio.com",
    ...
    notifications       = {
      "landing":        "Entering a pre-existing username will allow you to log in with your password.",
      "error":          "Error: An unexpected error occured. Please reload the page.",
      "invalid":        "Error: Your password is incorrect or you have been banned.",
      "loggedIn":       "Error: This user is already logged in.",
      "lockedOut":      "Error: You have attempted to log in too many times, please come back later.",
      "duplicateChat":  "Error: there is already a chat with this name. Choose a different name."
    },

    loginButtonText = {
      "new":            "Sign up",
      "existing":       "Log in",
      "cantdo":         "You cannot make an account with that name."
    };
...
template: '<hr><h3 class="usersOnline">{{users | usersOnline - 1 }} users currently online</h3>'
,
...
var login = Vue.extend({
  ...
  '<h2 class="login">Log in or sign up below.</h2>' + 
  '<p class="notification {{ notificationStatus }}">{{ notification }}</p>' +
  '<input class="" @keyup="isExistingUser()" @keyup.entnameInputer="goToChatrooms()" v-model="newUser.username" placeholder="Enter username..."/>' +
  '<input class="nameInput" type="password"  @keyup.tab="isExistingUser()" @keyup.enter="goToChatrooms()" v-model="newUser.password" placeholder="Enter password..."/>' +
...
var chatrooms = Vue.extend({
  ...
  '<input @keyup.esc="resetFilter()" v-model="searchString" type="text" placeholder="Search existing chatrooms..."/>' +
  '<p>OR</p>' + 
  '<button @click="toggleNewChatroomModal()">Create a chatroom</button>' +
  '<hr><h3 class="usersOnline">{{ users | usersOnline }} users currently online</h3><hr><div class="chatroomFlexContainer">' +
  '<div class="chatroom" v-for="chatroom in chatrooms | filterBy searchString" @click="goToMessages(chatroom)">' +
  
   '<i v-if="chatroom.creator == myUsername" @click.stop="console.log("test")" class="fa fa-close"></i>' +
  
  '<i v-if="chatroom.creator == myUsername" @click.stop="doNothing(chatroom[\'.key\'])" class="fa fa-close"></i>' +
  '<i v-if="chatroom.creator == myUsername" @click.stop="toggleEditChatroomModal(chatroom)" class="fa fa-cog"></i>' +
  '<h3>{{ chatroom.name }}</h3>' +
   '<p>Creator: {{ chatroom.creator }}</p>' +
...
var messenger = Vue.extend({
  ...
  '<span style="font-color:white;">@{{ message.username | isMyMessage }} <time class="hidden">sent at {{ message.timestamp }}</time></span>' +
  '<p>{{ message.text }}</p>' +
